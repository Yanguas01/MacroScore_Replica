...
class UserRepositoryImpl @Inject constructor(
    ...
    override suspend fun updateMyPassword(userUpdatePasswordRequest: UserUpdatePasswordRequest): Result<Unit> {
        return runCatching {
            val response = macroScoreApiService.editMyPassword(userUpdatePasswordRequest.toDTO())
    ...
    override suspend fun deleteMealFromSavedMeals(mealName: String): Result<Unit> {
        return runCatching {
            val response = macroScoreApiService.deleteMealFromSavedMeals(mealName)

            if (response.isSuccessful) {
                val userId = userManager.getUserId()
                mealDAO.deleteMealByMealName(userId, mealName)
            } else {
                throw Exception("Server error: ${response.code()} - ${response.message()}")
            }
        }
